...
const buildDir = (dir, out) => {
  ...
  pages.forEach((p) => {
    ...
    if (!p.startsWith(tDir)) {
      let html = buildTemplate(p, undefined, tDir, Deps);
      _write(out, relPath(dir, p), html);
    }
  });
};
        ...
        if (fs.existsSync(file) && !fs.statSync(file).isDirectory()) {
          _gatherDeps(Deps, file).forEach(f => {
            if (!f.startsWith(newPath(dir, "/templates"))) {
              let html = buildTemplate(f, undefined, newPath(dir, "/templates"), Deps);
              _write(out, relPath(dir, f), html);
            }
          });
        }
      } else {
